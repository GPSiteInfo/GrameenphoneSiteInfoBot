...
from GPSiteInfoBot import dispatcher, LOGGER, OWNER_ID, JOIN_LOGGER, SUPPORT_CHAT
from GPSiteInfoBot.__main__ import DATA_IMPORT
from GPSiteInfoBot.modules.helper_funcs.chat_status import user_admin
from GPSiteInfoBot.modules.helper_funcs.alternate import typing_action

# from GPSiteInfoBot.modules.rules import get_rules
import GPSiteInfoBot.modules.sql.rules_sql as rulessql

# from GPSiteInfoBot.modules.sql import warns_sql as warnssql
import GPSiteInfoBot.modules.sql.blacklist_sql as blacklistsql
from GPSiteInfoBot.modules.sql import disable_sql as disabledsql

# from GPSiteInfoBot.modules.sql import cust_filters_sql as filtersql
# import GPSiteInfoBot.modules.sql.welcome_sql as welcsql
import GPSiteInfoBot.modules.sql.locks_sql as locksql
from GPSiteInfoBot.modules.connection import connected
...
def export_data(update, context):
    ...
    with open("GPSiteInfoBot{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("GPSiteInfoBot{}.backup".format(chat_id), "rb"),
        caption="*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `GPSiteInfoBot-Backup` was specially made for notes.".format(
            chat.title, chat_id, tgl),
        timeout=360,
        reply_to_message_id=msg.message_id,
        parse_mode=ParseMode.MARKDOWN)
    os.remove("GPSiteInfoBot{}.backup".format(chat_id))  # Cleaning file
